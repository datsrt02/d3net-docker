...
async def knx_d3net_sync(body: D3netKnxLinkRequest):
    ...
    rows=await runtime.units_json_async() if runtime.gateway else []
    ...
    for t in body.targets:
        ...
        if not indoor or regs is None: continue
        ...
        for field,ga,val,dpt in vals:
            if knx_runtime.publish_group_value(str(ga),val,dpt,force=body.force): published.append({'target':t.get('target'),'indoor':indoor,'field':field,'ga':ga,'dpt':dpt,'value':val})
